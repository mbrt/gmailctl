...
func Diff(local, upstream GmailConfig, debugInfo bool, contextLines int, allowColor bool) (ConfigDiff, error) {
	res := ConfigDiff{
		LocalConfig: local,
	}
	var err error

	res.FiltersDiff, err = filter.Diff(upstream.Filters, local.Filters, debugInfo, contextLines, allowColor)
	if err != nil {
